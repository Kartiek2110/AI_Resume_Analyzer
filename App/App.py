...
def insert_data(sec_token,ip_add,host_name,dev_user,os_name_ver,latlong,city,state,country,act_name,act_mail,act_mob,name,email,res_score,timestamp,no_of_pages,reco_field,cand_level,skills,recommended_skills,courses,pdf_name):
    if user_data_collection:
        ...
        st.warning("MongoDB connection not available. Data not saved.")
...
def insertf_data(feed_name,feed_email,feed_score,comments,Timestamp):
    if user_feedback_collection:
        try:
            ...
            st.success("Feedback saved to MongoDB successfully!")
        except Exception as e:
            st.error(f"Failed to save feedback: {e}")
    else:
        st.warning("MongoDB connection not available. Feedback not saved.")
        ...
        if user_feedback_collection:
            try:
                ...
                if feedback_list:
                    ...
                    plotfeed_data = pd.DataFrame(feedback_list)
                    
                    # Rename columns for better display
                    plotfeed_data = plotfeed_data.rename(columns={
                        'feed_score': 'feed_score',
                        'feed_name': 'feed_name',
                        'comments': 'comments'
                    })
                    ...
                    if 'feed_name' in plotfeed_data.columns and 'comments' in plotfeed_data.columns:
                        st.subheader("**User Comment's**")
                        comment_df = plotfeed_data[['feed_name', 'comments']].rename(columns={'feed_name': 'User', 'comments': 'Comment'})
                        st.dataframe(comment_df, width=1000)
                else:
                    st.info("No feedback data found in the database.")
            except Exception as e:
                st.error(f"Error fetching feedback data: {e}")
        else:
            st.warning("MongoDB connection not available. Cannot fetch feedback data.")
                ...
                if user_data_collection:
                    try:
                        ...
                        if user_data_list:
                            ...
                            values = len(user_data_list)
                            st.success(f"Welcome Deepak ! Total {values} User's Have Used Our Tool : )")
                            
                            ### Fetch user data and convert it into dataframe
                            ...
                            df_display = df.rename(columns={
                                ...
                                'act_name': 'Name',
                                'act_mail': 'Mail',
                                'act_mob': 'Mobile Number',
                                'reco_field': 'Predicted Field',
                                'timestamp': 'Timestamp',
                                'name': 'Predicted Name',
                                'email': 'Predicted Mail',
                                'resume_score': 'Resume Score',
                                'no_of_pages': 'Total Page',
                                'pdf_name': 'File Name',
                                'cand_level': 'User Level',
                                ...
                                'country': 'Country',
                                'latlong': 'Lat Long',
                                'os_name_ver': 'Server OS',
                                'host_name': 'Server Name',
                                'dev_user': 'Server User'
                            })
                            ...
                            st.markdown(get_csv_download_link(df_display,'User_Data.csv','Download Report'), unsafe_allow_html=True)
                            
                            ### Analyzing All the Data's in pie charts
                            
                            # Pie chart for predicted field recommendations
                            if 'Predicted Field' in df_display.columns:
                                labels = df_display['Predicted Field'].unique()
                                values = df_display['Predicted Field'].value_counts()
                                st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                                fig = px.pie(df_display, values=values, names=labels, title='Predicted Field according to the Skills 👽', color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
                                st.plotly_chart(fig)
                            
                            # Pie chart for User's Experienced Level
                            if 'User Level' in df_display.columns:
                                labels = df_display['User Level'].unique()
                                values = df_display['User Level'].value_counts()
                                st.subheader("**Pie-Chart for User's Experienced Level**")
                                fig = px.pie(df_display, values=values, names=labels, title="Pie-Chart 📈 for User's 👨‍💻 Experienced Level", color_discrete_sequence=px.colors.sequential.RdBu)
                                st.plotly_chart(fig)
                            
                            # Pie chart for Resume Score
                            if 'Resume Score' in df_display.columns:
                                labels = df_display['Resume Score'].unique()
                                values = df_display['Resume Score'].value_counts()
                                st.subheader("**Pie-Chart for Resume Score**")
                                fig = px.pie(df_display, values=values, names=labels, title='From 1 to 100 💯', color_discrete_sequence=px.colors.sequential.Agsunset)
                                st.plotly_chart(fig)
                            
                            # Pie chart for Users
                            if 'IP Address' in df_display.columns:
                                labels = df_display['IP Address'].unique()
                                values = df_display['IP Address'].value_counts()
                                st.subheader("**Pie-Chart for Users App Used Count**")
                                fig = px.pie(df_display, values=values, names=labels, title='Usage Based On IP Address 👥', color_discrete_sequence=px.colors.sequential.matter_r)
                                st.plotly_chart(fig)
                            
                            # Pie chart for City
                            if 'City' in df_display.columns:
                                labels = df_display['City'].unique()
                                values = df_display['City'].value_counts()
                                st.subheader("**Pie-Chart for City**")
                                fig = px.pie(df_display, values=values, names=labels, title='Usage Based On City 🌆', color_discrete_sequence=px.colors.sequential.Jet)
                                st.plotly_chart(fig)
                            
                            # Pie chart for State
                            if 'State' in df_display.columns:
                                labels = df_display['State'].unique()
                                values = df_display['State'].value_counts()
                                st.subheader("**Pie-Chart for State**")
                                fig = px.pie(df_display, values=values, names=labels, title='Usage Based on State 🚉', color_discrete_sequence=px.colors.sequential.PuBu_r)
                                st.plotly_chart(fig)
                            
                            # Pie chart for Country
                            if 'Country' in df_display.columns:
                                labels = df_display['Country'].unique()
                                values = df_display['Country'].value_counts()
                                st.subheader("**Pie-Chart for Country**")
                                fig = px.pie(df_display, values=values, names=labels, title='Usage Based on Country 🌏', color_discrete_sequence=px.colors.sequential.Purpor_r)
                                st.plotly_chart(fig)
                        else:
                ...
                if user_feedback_collection:
                    try:
                        ...
                        if feedback_list:
                            ...
                            # Rename columns for better display
                            feedback_df_display = feedback_df.rename(columns={
                                '_id': 'ID',
                                'feed_name': 'Name',
                                'feed_email': 'Email',
                                'feed_score': 'Feedback Score',
                                'comments': 'Comments',
                                'timestamp': 'Timestamp'
                            })
                            
                            st.dataframe(feedback_df_display)
                            
                            ### Analyzing Feedback Data in pie charts
                            
                            # Pie chart for user ratings
                            if 'Feedback Score' in feedback_df_display.columns:
                                labels = feedback_df_display['Feedback Score'].unique()
                                values = feedback_df_display['Feedback Score'].value_counts()
                                st.subheader("**User Rating's**")
                                fig = px.pie(values=values, names=labels, title="Chart of User Rating Score From 1 - 5 🤗", color_discrete_sequence=px.colors.sequential.Aggrnyl)
                                st.plotly_chart(fig)
                            
                            # Fetching Comment History
                            if 'Name' in feedback_df_display.columns and 'Comments' in feedback_df_display.columns:
                                st.subheader("**User Comment's**")
                                comment_df = feedback_df_display[['Name', 'Comments']].rename(columns={'Name': 'User', 'Comments': 'Comment'})
                                st.dataframe(comment_df, width=1000)
                        else:
                            st.info("No feedback data found in the database.")
                    except Exception as e:
